...
![Radix](0001.two-sum.radix.png)

``` c
#include <stdio.h>
#include <malloc.h>

void encode_indices(int size, uint* src, unsigned long* dst) {
    for (unsigned long i = 0; i < size; i++) {
        dst[i] = src[i] | (i << 32);
    }
}

void radix_sort_into(int* histogram, int size, unsigned long* src, unsigned long* dst) {
